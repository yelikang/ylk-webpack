...
class NormalModule extends Module {
	...
	_doBuild(options, compilation, resolver, fs, hooks, callback) {
		...
		const processResult = (err, result) => {
			...
			// 源码source内容，构造Source对象(子类RawSource)
		...
		// doBuild中会调用合适的loader去加载指定的文件(可以是js、也可以是ts、less等)，装换为js，因为webpack只识别js模块
		// loader像一个信息管道流，接收字符串流、进行加工、返回加工后的字符串
		runLoaders(
			{
	...
	build(options, compilation, resolver, fs, callback) {
		...
		// 获取当前compilation下当前NormalModule的hooks，
		...
		return this._doBuild(options, compilation, resolver, fs, hooks, err => {
			...
			try {
				const source = this._source.source();
				// build完之后，调用第三方的acorn对js源代码进行语法解析（会进行dependencies依赖解析）

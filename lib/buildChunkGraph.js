...
const visitModules = (
	...
	const getBlockModules = (block, runtime) => {
		...
		if (memCache !== undefined) {
			...
			logger.time("visitModules: prepare");
			// 获取每个block中(Module中)依赖的Modules(实际上是从moduleGraph的_modules依赖关系中获取)
	 ...
	 * @returns {void}
	 * 找到module依赖的Modules,不断压入到队列中去处理，直到最后没有新的依赖module压入队列
	 */
	...
	// 处理队列，每个module会有不同的状态action，不同状态处理不同逻辑；
	// 状态1: 建立chunk与EntryModule的连续
	// 状态2: 状态设置为5，重新放入队列
	// 状态3: 处理module中的block依赖；依赖会循环进入队列，依赖处理完之后，再逐步回退处理上级Module
	// 状态4：处理异步DependenciesBlock
	// 状态5: 退出
	const processQueue = () => {
		while (queue.length) {
			...
			switch (queueItem.action) {
				...
				// fallthrough 3:处理Module的blcoK依赖
				case PROCESS_BLOCK: {
					// 找到block(也就是Module)的依赖，转化为queueItem，压入栈
					processBlock(block);
					break;
				}
...
const buildChunkGraph = (compilation, inputEntrypointsAndModules) => {
	...
	// 这里会解析modules，及每个module依赖的modules，建立chunk与modules的关系
	visitModules(
		logger,
		compilation,
		inputEntrypointsAndModules,

...
BREAKING CHANGE: Asset processing hooks in Compilation has been merged into a single Compilation.hooks.processAssets hook.`;
		...
		// Module图谱
		// ModuleGraph 记录每个模块及其直接和间接依赖关系。它保存的是模块级别的依赖网络。
		this.moduleGraph = new ModuleGraph();
		/** @type {ChunkGraph} */
		// Chunk图谱
		// ChunkGraph 是在 ModuleGraph 的基础上,根据代码分割规则 further group 模块形成代码块(chunk)。
	...
	_processModuleDependencies(module, callback) {
		...
		const onDependenciesSorted = err => {
			...
			// 循环处理module中的dependences依赖，构建依赖的Module
		...
		try {
			...
			do {
				...
				if (block.dependencies) {
					currentBlock = block;
					let i = 0;
					// 循环Module中的dependencies，构建sortedDependencies(排序)
			...
			(err, factoryResult) => {
				const applyFactoryResultDependencies = () => {
					const { fileDependencies, contextDependencies, missingDependencies } =
						factoryResult;
					// 添加每次构建的依赖文件（fileDependencies解析在NormalModuleFactory中 => .hooks.factorize => .hooks.resolve => defaultResolve => resolveResource =>  enhanced-resolve中进行）
					// enhanced-resolve只是做文件路径解析，不会做文件内容解析
					// 所以首先是在入口文件内容，进行ast语法解析时获取Module的dependencies，
					// 循环dependencies 内容，通过enhanced-resolve加载每个dep文件内容，构建fileDependencies的文件依赖信息
					if (fileDependencies) {
	...
	_handleModuleBuildAndDependencies(originModule, module, recursive, callback) {
		...
		this.buildModule(module, err => {
			...
			// 构建完module之后，开始处理module中的依赖；dependencies数据会在buildModule中获取到（在NormalModule的_doBuild中）;
			// 最终会调用_processModuleDependencies，循环处理module中的dependences依赖
	...
	seal(callback) {
		...
		// 是否向后兼容
		if (this._backCompat) {
			// 遍历compilation.modules集合，将module按entry/动态引入的规则分配给不同的chunk对象；得到完整的chunks集合对象，调用createChunkAssets方法
			// 规则
			// 1.entry及entry触达的模块，组装成一个chunk
			// 2.使用动态引入语句引入的模块，各自组合成一个chunk (查看动态的依赖在module的Dependence中的表象???)
			for (const module of this.modules) {
				// 记录在ChunkGraph的全局对象上WeakMap，每个module都指向相同的chunkGraph
		...
		// 遍历entries入口，entries入口信息在_addEntryItem中添加
		for (const [name, { dependencies, includeDependencies, options }] of this
			...
			// chunk push进entrypoint(ChunkGroup)的chunks中；并且chunk中记录chunkGroup
...
Or do you want to use the entrypoints '${name}' and '${runtime}' independently on the same page with a shared runtime? In this case give them both the same value for the 'runtime' option. It must be a name not already used by an entrypoint.`);
		...
		// 构建ChunkGraph，建立chunk与module的关系
		...
		// 优化chunks的钩子函数(SplitChunksPlugin注册了该钩子)
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			this.hooks.optimizeChunkModules.callAsync(
				...
				err => {
					...
					// 这里会创建异步chunk的名称，在NamedChunkIdsPlugin中监听的
	...
	Make sure to select an appropriate stage from Compilation.PROCESS_ASSETS_STAGE_*.`
							...
							if (this.hooks.shouldGenerateChunkAssets.call() !== false) {
								...
								// 遍历chunks，创建assets，记录到compilation.assets

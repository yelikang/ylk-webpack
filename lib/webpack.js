...
const createCompiler = rawOptions => {
	...
	// 注册所有自定义插件对象(对象构建时在webpack.config中new出来的)；如果插件是函数，直接调用，否则调用对象的apply方法
	// 注册插件其实是通过tapable进行各种回调事件的注册
	if (Array.isArray(options.plugins)) {
		// 循环执行所有插件的apply方法(如果插件为function,直接call)
	...
	// process执行初始化工作，根据配置项注入各种默认plugin插件
...
const webpack = /** @type {WebpackFunctionSingle & WebpackFunctionMulti} */ (
	...
	(options, callback) => {
		...
		// 如果指定了callback，这直接执行compiler的run方法；否则返回compiler，用户手动指定run方法
		if (callback) {
			try {

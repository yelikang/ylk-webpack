...
/** @typedef {import("estree").FunctionDeclaration} FunctionDeclarationNode */
/** @typedef {import("estree").FunctionExpression} FunctionExpressionNode */
/** @typedef {import("estree").ArrowFunctionExpression} ArrowFunctionExpressionNode */
/** @typedef {import("estree").BlockStatement} BlockStatementNode */
/** @typedef {import("./Compiler")} Compiler */
/** @typedef {ArrowFunctionExpressionNode | FunctionExpressionNode | FunctionDeclarationNode} FunctionNode */
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				const handler = parser => {
					parser.hooks.terminate.tap("ConstPlugin", statement => {
						/** @type {FunctionNode} */
						let fn;
						const prevStatements = parser.statementPath;
						let j = prevStatements.length - 2;
						// find nearest function statement
						for (; j > -1; j--) {
							const type = prevStatements[j].type;
							if (
						...
						const body = /** @type {BlockStatementNode} */ (fn.body);
						if (body.body[body.body.length - 1] === statement) return;

						let lastEnd = statement.range[1];
						for (let i = prevStatements.length - 2; i > j; i--) {
							const statement = prevStatements[i];
							...
							if (
								...
								const dep = new ConstDependency("", [lastEnd, end - 1]);

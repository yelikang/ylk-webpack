...
class ChunkGraphModule {
	constructor() {
		...
		// 当前Module被哪些入口chunks依赖(属于哪些chunk，只有入口Module才有这个值)
		this.entryInChunks = undefined;
		/** @type {Set<Chunk> | undefined} */
		// 当前Module被哪些运行时chunks依赖(s属于哪些chunk)
...
class ChunkGraphChunk {
	constructor() {
		...
		// 当前chunk依赖哪些入口Module(只有入口chunk才有该值)
		this.entryModules = new Map();
		/** @type {SortableSet<RuntimeModule>} */
		// 当前chunk依赖哪些运行时Module
...
class ChunkGraph {
	...
	connectChunkAndEntryModule(chunk, module, entrypoint) {
		...
		// 记录当前module属于哪些chunk(entryChunk)
		cgm.entryInChunks.add(chunk);
		// 记录当前chunk里面有哪些module(entryModule)

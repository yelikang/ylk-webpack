...
${other}`);
		...
		this.hooks.emit.callAsync(compilation, err => {
			...
			// 创建输入目录，之后emitFiles回调；outputFileSystem文件存储能力，在NodeEnvironmentPlugin中赋予的
	...
	compile(callback) {
		...
		// 1.构建阶段，从入口文件递归循环，得到每个模块翻译后的内容和生成依赖关系图；构建module[a]、module[b]依赖对象
		this.hooks.beforeCompile.callAsync(params, err => {
			if (err) return callback(err);

			this.hooks.compile.call(params);

			// 构建compilation对象(每次编译作业的环境)
			const compilation = this.newCompilation(params);

			const logger = compilation.getLogger("webpack.Compiler");

			logger.time("make hook");
			// 2.真正的编译流程发生在make阶段(添加entry入口)，调用EntryPlugin中的make
				...
				this.hooks.finishMake.callAsync(compilation, err => {
					...
					process.nextTick(() => {
						...
						compilation.finish(err => {
							...
							// 3.构建阶段结束，开始生成阶段seal(seal: 密封、上锁)；即将module到chunks的转换
